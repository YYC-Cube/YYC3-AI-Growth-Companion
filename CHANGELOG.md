...
### 修复
- **旗舰页空壳（用户实测反馈）**：middleware 此前把 / 与 /growth 交给语言路由，被 [locale] 子树的超简化镜像页（208/425 行）遮蔽根目录完整版（421/731 行），首页与成长页渲染成 75 字符空壳——语言路由现仅作用于显式语言前缀（/en、/zh/...），根路径一律直出完整版。实测：首页 75→1364 字符/31 按钮，成长页 75→1806 字符/15 标题
- 主题深化：非默认主题接管渐变背景/边框/卡片文字/琥珀动作色（赛博下动作色转霓虹青、液态下转翡翠），深色主题文字可读性全面修复
