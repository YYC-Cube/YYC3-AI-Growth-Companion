...
const GrowthTreePage: React.FC<{ onNavigate?: (page: string, data?: any) => void }> = ({ onNavigate }) => {
  // const navigate = useNavigate(); // Removed to use prop based navigation
  const navigate = (path: string) => {
     // Map path to page key for App.tsx
     if (path.startsWith('/growth/record/')) {
         // This is a detail page, we don't have it in App.tsx yet, maybe just log or alert
         console.log("Navigate to detail", path);
     } else if (path === '/growth/cultural') {
         onNavigate?.('culture');
     } else if (path === '/growth/milestones') {
         onNavigate?.('badges'); // Assuming milestones map to badges or similar
     } else {
